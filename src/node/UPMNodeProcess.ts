...
> {
  ...
  private onServiceClientPort(clientId: string, port: Electron.MessagePortMain) {
    ...
    port.on("message", (ev: Electron.MessageEvent) => {
      const envelope = (ev.data?.message ?? ev.data) as UPM.NodeEnvelope,
  ...
  private onMessage(port: UPM.Port, message: Electron.MessageEvent) {
    ...
    const {channel, payload} = (message.data?.message ?? message.data) as UPM.NodeEnvelope
    match(channel)
        .when(c => c === UPM.IPCChannel.UPMServiceMessage, () => this.onServiceMessage("main", port, payload))
        .when(c => c === UPM.IPCChannel.UPMServiceNewClient, () => this.onServiceClientPort(payload.clientId, message.ports[0]))
        .otherwise(c => {
